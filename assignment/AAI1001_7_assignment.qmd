...
## Outliers
IDK if there is actually a need to deal with the outliers or how to. below is just to show the time series trends in the data for inspection (remove this previous when submitting)
...
# Combine work and not working into one tibble
labour_status <- full_join(
  work_clean, 
  not_working_clean, 
  by = c("year", "sex", "marital_status", "age_band")
)

# Now join to fertility
fertility_labour_joined <- fertility_clean |>
  left_join(labour_status, by = c("year", "age_band"))

# pivot longer to have a single count column for labour status
fertility_labour_long <- fertility_labour_joined |>
  pivot_longer(
    cols = c("labour_force", "outside_labour_force"),
    names_to = "labour_status",
    values_to = "count_thousands"
  )
```

The final tibble "full_dataset" is ready for visualisation. for now i have left the sex for male and female in, i do not think we will need male.
