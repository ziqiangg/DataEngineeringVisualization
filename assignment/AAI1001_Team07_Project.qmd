...
# Wrap into SharedData objects (same group = filters apply to both)
shared_fertility <- SharedData$new(fertility_data, group = "common_group")
shared_bar <- SharedData$new(bar_data, group = "common_group")

# Filter controls
filter_marital <- filter_select("filter_marital", "Marital Status:", shared_fertility, ~marital_status)
filter_labour  <- filter_select("filter_labour", "Labour Status:", shared_fertility, ~labour_status)
filter_ageband <- filter_select("filter_ageband", "Age Band:", shared_fertility, ~age_band)

# Colors
marital_colors <- setNames(brewer.pal(length(unique(bar_data$marital_status)), "Set2"),
                           unique(bar_data$marital_status))
age_colors <- setNames(brewer.pal(length(unique(fertility_data$age_band)), "Dark2"),
                       unique(fertility_data$age_band))

# Scaling factor for dual axis
scale_factor <- max(bar_data$total_count, na.rm = TRUE) / max(fertility_data$fertility_rate, na.rm = TRUE)

# Combined plot
p <- ggplot() +
  # Bar plot (stacked by marital status, dodged by labour status)
  geom_col(
    data = shared_bar,
    aes(
      x = as.factor(year),
  ...
  # Fertility rate line and points
  geom_line(
