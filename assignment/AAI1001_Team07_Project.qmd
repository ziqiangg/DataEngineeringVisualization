...
- Dodged bar chart for age-banded population data (by age band, marital status, labour status & year) with overlaying age-banded fertility rate lines

- Faceted dashboard for clear analysis of individual dimensions (clearly visualises total population data by labour status, marital status and year)
...
- **Socio-Economic Factors**: The original visualisation lacked explanatory power for Singapore’s fertility decline. Our dashboard introduces dimensions like **female population** allowing users to evaluate female **labour status**, and **marital status** to reveal potential **causal relationships** and enrich policy relevance.

- **Age-Banded Fertility**: Instead of a single flat fertility rate, we analyse age-banded fertility rates  for **age bands 15–49 in 5-year increments**. This highlights how fertility behaviour shifts across cohorts and identifies *which age groups contribute most* to the decline.

---

## test
trying to create total female population visualisation instead of by ageband, which is more clear for our presentation purposes. this is higher level and not so detailed as the finished dodged one. 
**issues** this current total fertility line and points do not persist throughout all combinations of marital andlaboour status dropdowns, otherwise ok
```{r}
# Load and clean data
filtered_dataset <- final_dataset |>
  filter(age_band == "All") |>
  mutate(
    ...
    count           = as.numeric(as.character(count)),
    fertility_rate  = as.numeric(as.character(fertility_rate)),
    uom             = "per female"  # Total fertility rates for "All" age band
  )

# SharedData for interactivity (filtering out 1995, 2000, and 2005 for the lines)
shared_data <- SharedData$new(filtered_dataset, group = "full_filter")

# Filter controls (only marital status and labour status dropdowns)
filter_marital <- filter_select("filter_marital", "Marital Status:", shared_data, ~marital_status)
filter_labour  <- filter_select("filter_labour", "Labour Status:", shared_data, ~labour_status)
...
# Scaling factor (used to adjust the scales of fertility rate and count to display on the same graph)
scale_factor <- max(filtered_dataset$count, na.rm = TRUE) / max(filtered_dataset$fertility_rate, na.rm = TRUE)
...
p <- ggplot() +
  geom_col(
    ...
    aes(
      x = factor(year),
      y = count,  # Use the original count column for female population
      fill = marital_status,  # Segments by marital status
      group = interaction(labour_status, marital_status),  # Ensure the bars are grouped by both labour and marital status
      text = paste0(
        "Year: ", year,
        "<br>Marital Status: ", marital_status,
        "<br>Labour Status: ", labour_status,
        "<br>Female Population: ", comma(count)  # Use the original female population count
      )
    ),
    position = position_dodge(width = 0.7),  # Keep the dodge position for bars
    width = 0.6,
    colour = "white",
    size = 0.2,
    alpha = 0.6  # Reduced alpha for better visibility of segments
  ) +
  # Fixed Color for Fertility Rate Line (excluding 1995, 2000, and 2005)
  geom_line(
    data = filtered_dataset |> filter(!year %in% c(1995, 2000, 2005)),  # Exclude these years for the line plot
    aes(
      x = as.factor(year),
      y = fertility_rate * scale_factor,  # Adjust the fertility rate to match the count scale
      group = marital_status,
      colour = "Fertility Rate"  # Set a fixed color for fertility rate line
    ),
    size = 0.8,  # Reduced line size for a more subtle appearance
    alpha = 0.6  # Reduced alpha for the line
  ) +
  # Points for fertility rates (including gray points for 1995, 2000, and 2005)
  geom_point(
    ...
    aes(
      ...
      y = fertility_rate * scale_factor,  # Adjust the fertility rate to match the count scale
      group = marital_status,
      colour = ifelse(year %in% c(1995, 2000, 2005), "gray", "Fertility Rate"),  # Gray for 1995, 2000, 2005
      text = paste0(
        "Year: ", year,
        "<br>Fertility Rate: ", round(fertility_rate, 2),
        "<br>Unit: ", uom  # Added the unit of measurement (uom) to the hover tooltip
  ...
  scale_fill_manual(values = marital_colors) +  # Apply custom colors for marital status
  scale_colour_manual(values = c("Fertility Rate" = "#FF6347", "gray" = "#808080")) +  # Set fixed color for fertility rate line/points (Tomato and Gray)
  scale_y_continuous(
    name = "Female Population (thousands)",
    labels = comma
  ) +
  labs(
    title = "Fertility Rates vs Labour Participation",
    subtitle = "Bars: Female Population Segmented by Marital Status and Labour Status; Points: Fertility Rates by Marital Status",
    x = "Year",
    caption = "Source: SingStat & data.gov.sg"
  ) +
  theme_minimal() +
...
tagList(
  div(
    style = "margin-bottom:20px;",
    h3("Interactive Fertility & Labour Plot"),
    p("Use filters to explore aggregated female population and fertility rates.")
  ),
  div(
    style = "display: flex; gap: 15px; margin-bottom: 20px;",
    filter_marital,
...
```
persistent total fertility rate line, but the 1995, 2000 and 2005 points do not persist also they should not be connected with lines, should be like plot above
```{r}
# Load and clean data
filtered_dataset <- final_dataset |>
  filter(age_band == "All") |>
...
# SharedData for interactivity (keeping all combinations of marital and labour status)
shared_data <- SharedData$new(filtered_dataset, group = "full_filter")

# Filter controls (marital status and labour status dropdowns)
filter_marital <- filter_select("filter_marital", "Marital Status:", shared_data, ~marital_status)
filter_labour  <- filter_select("filter_labour", "Labour Status:", shared_data, ~labour_status)
...
p <- ggplot() +
  ...
  # Fertility Rate Line (across all dropdown selections)
  geom_line(
    data = shared_data,
    aes(
      x = as.factor(year),
      y = fertility_rate * scale_factor,  # Adjust the fertility rate to match the count scale
      group = marital_status,
      colour = "Fertility Rate"  # Fixed color for fertility rate line
    ),
    size = 0.8,  # Reduced line size for a more subtle appearance
    alpha = 0.6  # Reduced alpha for the line
  ) +
  # Points for fertility rates
  geom_point(
    data = shared_data,
    aes(
      x = as.factor(year),
  ...
  scale_fill_manual(values = marital_colors) +  # Apply custom colors for marital status
  scale_colour_manual(values = c("Fertility Rate" = "#FF6347")) +  # Fixed color for fertility rate line/points
  scale_y_continuous(
    name = "Female Population (thousands)",
    labels = comma
  ) +
...
Built using the `ggiraph` library, enabling:

- Zooming
- Tooltip inspection
- Lasso selection or on-click comparison
...
- Faceting relevant dimensions (female population, labour status, fertility rates and year) to provide user dedicated inspection if desired
...
**LM Slope**
- From the simple regression TFR ~ LFPR. A one-unit (i.e., 100 percentage-point) rise in LFPR would predict a 3.447 drop in TFR; scaled more realistically, each 10 pp rise in LFPR ↘ TFR by about 0.345. The slope is *highly significant*, confirming that LFPR is a strong linear predictor of fertility.
