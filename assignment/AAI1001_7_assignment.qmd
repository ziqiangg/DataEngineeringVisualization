...
2. **It does not account for other socio-economic variables such as income, work stress, cost of living, or gender disparities.**
...
3. **Gender**: Highlight the differences in fertility and burden by gender.
...
# IQR calculation
Q1 <- quantile(fertility_age_clean$births, 0.25, na.rm = TRUE)
Q3 <- quantile(fertility_age_clean$births, 0.75, na.rm = TRUE)
IQR <- Q3 - Q1
...
outliers <- fertility_age_clean %>%
  filter(births < lower_bound | births > upper_bound) %>%
  arrange(births)

# Visualise outliers
ggplot(fertility_age_clean, aes(x = value)) +
  geom_boxplot(outlier.color = "red", na.rm = TRUE) +
  labs(
    title = "Fertility Rate Outliers (IQR Method)",
    x = "Fertility Rate (Births Per Thousand Females)"
  ) +
  theme_minimal()
...
print(nrow(outliers))
```
From the IQR Boxplot, we can conclude that there are 18 observations that are considered outliers above the upper bound of the dataset.
