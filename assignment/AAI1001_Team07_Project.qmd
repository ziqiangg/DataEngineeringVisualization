  ...
  geom_point(
    ...
    aes(
      ...
      text = paste0(
        "Year: ", year,
        "<br>Age Band: ", age_band,
        "<br>Fertility Rate: ", round(fertility_rate, 2)  # Show fertility rate for age band
      )
    ),
    size = 1,  # Reduced point size
...
# Legend for Marital Status and Age Bands
labour_legend <- paste(
  '<div style="font-weight:bold;margin-bottom:8px;">Marital Status</div>',
  paste(sapply(names(marital_colors), function(label) {
    sprintf('<div style="margin-bottom:4px;"><span style="display:inline-block;width:12px;height:12px;background:%s;margin-right:6px;"></span><span style="font-size:12px;">%s</span></div>',
            marital_colors[[label]], label)
  }), collapse = "")
)
...
this is the ggplot and plotly which can filter by all dimensions but is unable to do ages correctly only 15-19 is appearing and we do not know why the other ages are not hiding / showing properly. (robin in the future i think it is becuz the summarising is done only for 15-19)

```{r}
#–– 1. Wrap the entire final_dataset in one SharedData object
shared_data <- SharedData$new(final_dataset, group = "common_group")

#–– 2. Pull it back out and clean
full <- shared_data$data() %>%
  mutate(
    marital_status = tools::toTitleCase(trimws(as.character(marital_status))),
    labour_status  = tools::toTitleCase(trimws(as.character(labour_status))),
    age_band       = tools::toTitleCase(trimws(as.character(age_band))),
...
fertility_data <- full %>% filter(age_band != "All")
bar_data       <- full %>%
  filter(age_band == "All") %>%
  group_by(year, marital_status, labour_status) %>%
  summarise(total_count = sum(count, na.rm = TRUE), .groups = "drop")

#–– 4. Make SharedData for both plots
shared_fertility <- SharedData$new(fertility_data, group = "common_group")
shared_bar       <- SharedData$new(bar_data,       group = "common_group")

#–– 5. Build filters
#   Marital & Labour filter both plots
filter_marital <- filter_checkbox(
  id         = "filter_marital",
...
#   Age‐band filter ONLY fertility layer
filter_ageband <- filter_checkbox(
  id         = "filter_ageband",
  label      = "Age Band:",
  sharedData = shared_fertility,
  group      = ~age_band,
  inline     = FALSE,
  allLevels  = TRUE
)
...
#–– 6. Colors & scaling
marital_colors <- setNames(
  brewer.pal(length(unique(bar_data$marital_status)), "Set2"),
  unique(bar_data$marital_status)
)
age_colors <- setNames(
  brewer.pal(length(unique(fertility_data$age_band)), "Dark2"),
  unique(fertility_data$age_band)
)
scale_factor <- max(bar_data$total_count, na.rm = TRUE) /
                max(fertility_data$fertility_rate, na.rm = TRUE)
...
p <- ggplot() +
  ...
  scale_y_continuous(
    ...
    sec.axis = sec_axis(
      ~ . / scale_factor,
      name   = "Fertility Rate (per female)",
      labels = label_number(accuracy = 0.1)
    )
  ) +
  labs(
    title    = "Fertility Rates vs Labour Participation",
    subtitle = "Bars: Population by Marital & Labour Status; Lines: Fertility by Age Band",
    x        = "Year",
    caption  = "Source: SingStat & data.gov.sg"
  ) +
  theme_minimal() +
...
#–– 9. Legends
marital_legend <- HTML(paste0(
  '<div style="font-weight:bold;margin-bottom:8px;">Marital Status</div>',
  paste0(lapply(names(marital_colors), function(lbl) {
    sprintf(
      '<div style="margin-bottom:4px;"><span style="display:inline-block;width:12px;height:12px;background:%s;margin-right:6px;"></span>%s</div>',
      marital_colors[[lbl]], lbl
    )
  }), collapse = "")
))
age_legend <- HTML(paste0(
  '<div style="font-weight:bold;margin-top:15px;margin-bottom:8px;">Age Bands</div>',
  paste0(lapply(names(age_colors), function(lbl) {
    sprintf(
      '<div style="margin-bottom:4px;"><span style="display:inline-block;width:12px;height:12px;background:%s;margin-right:6px;"></span>%s</div>',
      age_colors[[lbl]], lbl
    )
...
combined_legend <- div(
  style = "margin-top:20px;padding:10px;background:#f9f9f9;border:1px solid #ccc;border-radius:5px;",
  marital_legend, age_legend
)
...
tagList(
  div(
    style = "margin-bottom:20px;",
    h3("Interactive Fertility & Labour Dashboard"),
    p("Use the checkboxes below to filter population counts and fertility rates.")
  ),
  div(
    style = "display:flex; flex-wrap:wrap; gap:15px; margin-bottom:20px;",
    filter_marital, filter_labour, filter_ageband
  ),
  plotly_output,
  combined_legend
)
