...
faceted ggiraph v2, the checkboxes not working
...
tooltip_df <- agg_counts %>%
  pivot_wider(names_from=labour_status, values_from=count, values_fill=list(count=0)) %>%
  rename(in_LF=labour_force, out_LF=outside_labour_force) %>%
  mutate(
    tooltip = paste0("Year: ",year,"<br/>",
                     "Marital Status: ",marital_status,"<br/>",
                     "In LF: ",comma(in_LF),"k<br/>",
                     "Out LF: ",comma(out_LF),"k"),
    bar_id = paste(year, marital_status)
  ) %>%
  pivot_longer(c(in_LF,out_LF), names_to="labour_status", values_to="count") %>%
  mutate(labour_status = recode(labour_status,
                                in_LF="labour_force",
...
scale_factor <- max(tooltip_df$count, na.rm=TRUE) / max(fertility_ab$fertility_rate, na.rm=TRUE)

# — CROSSTALK SHARED DATA —
counts_sd <- SharedData$new(tooltip_df,   key=~bar_id,  group="ms_filter")
fert_sd   <- SharedData$new(fertility_ab, key=~data_id, group="ms_filter")

# — BUILD THE CONTROL & PLOT —
marital_ctl <- filter_checkbox(
  id         = "marital_filter",
  label      = "Show Marital Status:",
  sharedData = counts_sd,
  group      = ~marital_status
)

bar_plot <- ggplot(counts_sd) +
  geom_col_interactive(aes(x=year, y=count, fill=marital_status,
                           data_id=bar_id, tooltip=tooltip),
  ...
  facet_wrap(~labour_status, ncol=1,
             labeller=labeller(labour_status=c(
...
line_plot <- ggplot(fert_sd) +
  geom_line_interactive(aes(x=year, y=fertility_rate*scale_factor, colour=age_band,
                            group=age_band, data_id=data_id, tooltip=tooltip), size=1) +
  geom_point_interactive(aes(x=year, y=fertility_rate*scale_factor, colour=age_band,
                             group=age_band, data_id=data_id, tooltip=tooltip), size=3) +
  scale_y_continuous(name="Fertility Rate × Scale Factor", labels=comma) +
  scale_colour_brewer(palette="Set2", name="Age Band") +
  theme_minimal(base_size=10) +
  theme(legend.position="bottom", strip.text=element_text(face="bold", size=12))

combined <- bar_plot + line_plot + plot_layout(ncol=1, heights=c(2,1))

gir <- girafe(
  ggobj     = combined, width_svg=12, height_svg=8,
  options   = list(
