...
- **Extended Analysis**: Compared to the 2019–2023 analysis of the original, our visualisation expands the scope to **fertility rates from 1990 to 2022**.

- **Socio-Economic Factors**: The original visualisation lacked explanatory power for Singapore’s fertility decline. Our dashboard introduces dimensions like **population**, **labour status**, and **marital status** to reveal potential **causal relationships** and enrich policy relevance.

- **Age-Banded Fertility: Instead of a single flat fertility rate, we analysed **age-specific fertility rates (ASFR)** for **age bands 15–49 in 5-year increments**. This highlights how fertility behaviour shifts across cohorts and identifies **which age groups contribute most** to the decline.
...
- Fully interactive with:
  ...
  - `Marital status`
  - `Labour status`
- Built using the `Plotly` library, enabling:
  - Zooming
  - Tooltip inspection
  - Dynamic filtering
...
# ── 1. Recreate LFPR at the YEAR level ────────────────────────────────────────
...
library(gt)
gt(test_results) |> 
  tab_header(title = md("**Statistical Significance Tests**"))
