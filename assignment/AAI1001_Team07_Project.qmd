...
# Visualisation
To properly deal with visual clutter of various dimensions, we will test 3 methods of time series visualisation to handle extensive female population data & intensive fertility rate data :

- Stacked bar chart for population data with overlaying fertility rate lines

- Dodged bar chart for population data with overlaying fertility rate lines

- Faceted dashboard for clear analysis of individual dimensions
...
tagList(
  div(style = "margin-bottom:20px;",
      h3("Stacked Interactive Fertility & Labour Plot"),
...
tagList(
  div(
    ...
    h3("Dodged Interactive Fertility & Labour Plot"),
    p("Use filters to explore female labour force counts and age-banded fertility rates.")
  ),
  div(
...
**Pearson r Test**:
Purpose: Measures the strength and direction of the linear relationship between two continuous variables. In this case, it quantifies how closely the female labour-force-participation rate (LFPR) and the total fertility rate (TFR) move together in a straight line.

**LM Slope** (from TFR ~ LFPR):
Purpose: Fits a simple linear regression model to estimate the quantitative impact (slope) of one variable (LFPR) on another (TFR). It tells you the expected change in TFR for a one-unit change in LFPR and assesses the statistical significance of this relationship.

**Kruskal–Wallis Test**:
Purpose: A non-parametric test used to determine if there are statistically significant differences in the distributions of a continuous variable (fertility rate) across three or more independent groups (age bands). 

**Chow F** (1998 break):
Purpose: Tests for a structural break in a regression model at a known point in time (1998). It checks whether the relationship between TFR and LFPR (the slope and/or intercept of the regression line) changed significantly before and after the Asian Crisis.

p-value is a probability that measures the strength of evidence against a null hypothesis "no effect" obtained from the data. 
```{r}
#| label: tbl-8
